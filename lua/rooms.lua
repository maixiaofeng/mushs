...
lookCitys = {
	...
	["caoyuan"] = "大草原",	["zhiye"] = "不知道哪里",		["group"] = "中原",	
	["entry"] = "",
}
...
function getRooms(name, city, type, room_rel)
    --print('getRooms.......'..name..','..city)
	local type = type or "all"
	local isLookCity = type == "all" or type == "look"
	local isTaskCity = type == "all" or type == "task"
	local rel = room_rel
	...
	local isCityFw = city and string.find(city,"^%l+$")
	---add by maixf 2020.1.21  ------------
	--[[if isNameFw ~= nil then 
	   print('isNameFw..:'..isNameFw)
    end	
	if isCityFw ~= nil then 
	   print('isCityFw..:'..isCityFw)
    end]]--
	--- add end --------------------------
	local rooms = {}
	for id,room in pairs(map.rooms) do
		...
		if roomName == name then
			if not city then
				table.insert(rooms,id)
		--[[elseif city == '不知道哪里' then 
			    local unknowcity = getCity(room.id)
				if unknowcity == '中原神州' then table.insert(rooms,id) end]]--
			else
				local lookCity = isCityFw and getFirstWord(getLookCity(room.id)) or getLookCity(room.id)
				local taskCity = isCityFw and getFirstWord(getCity(room.id)) or getCity(room.id)
				--print(lookCity,taskCity)
				if (isLookCity and city == lookCity) or (isTaskCity and city == taskCity) or (room.outdoor and room.outdoor == city) or (rel and room.room_relative and rel==room.room_relative) then
...
function getAroundRooms_ex(name, city, length, room_id, type)
    --print('getAroundRooms...'..name..','..city..','..length)
	local rooms = getRooms(name, city, type)
	local allRooms = {}
	for _, id in pairs(rooms) do
...
function SjMap:getAroundRooms(path, length)
	...
	for k, v in pairs(distances) do
	    if (job.name=='wudang' or job.name=='huashan') and flag.searchArea ~= nil then  
           for distance_v, search_areacode  in pairs(flag.searchArea) do
                 if string.find(k, search_areacode) and v <= tonumber(distance_v) then 
...
Room {
	...
	name = "大客栈",
	ways = {
		["#czkedianoutgosleep"] = "cangzhou/nanjie1",
		["#czkedianoutgosleep1"] = "cangzhou/beijie3",
		["#cangzhoukedian"] = "cangzhou/kezhan2",
	},
	lengths = {
		["#cangzhoukedian"] = 5,	
	},
...
Room {
	...
	room_relative="大驿道大驿道↖↗大驿道↘青石路大驿道",
}
Room {
	id = "changle/road2",
	name = "青石路",
...
Room {
	...
	room_relative="长江北岸---长江北岸长江北岸",
	room_relative2="长江渡船∧长江北岸---长江北岸长江北岸"
...
Room {
	...
	room_relative="长江南岸---长江南岸长江南岸",
	room_relative2="长江渡船∧长江南岸---长江南岸长江南岸"
}
Room {
	id = "city/jiangnan2",
	name = "长江南岸",
...
Room {
	...
	lengths = {
		...
		["#gosxh"] = "if yili_midnigth_judge() then return false else return 1 end",
		["#gozbd"] = "if yili_midnigth_judge() then return false else return 1 end",
	},
	room_relative="瘦西湖酒馆｜西门----西大街----西大街｜珠宝店西大街",
	room_relative2="西门----西大街----西大街西大街",
}
...
Room {
	...
	lengths = {
		["#jldin"] = "if Bag['火折'] then return 20 else return false end",
	},
	room_relative="【你现在正处于峨嵋山】九老洞九老洞",
}
Room {
	id = "emei/leidongping",
	name = "雷洞坪",
...
Room {
	...
	room_relative="雪山丛林↑雪山丛林←山溪边雪山丛林",
}
Room {
...
Room {
	...
	room_relative="川西土路↗藏边土路↙藏边土路藏边土路",
}
Room {
	...
	room_relative="湖边小路↖湖边小路---湖边小路湖边小路",
}
Room {
...
Room {
	...
	lengths = {
		...
		["s"] = "if score.party and score.party=='古墓派' then return false else return 1 end",
	},
	room_relative="山路↑山路｜山路山路",
}
Room {
...
Room {
	...
	ways = {
		...
		--["#inxiaofunew"] = "huanghe/yyd/dating",
	},
	nolooks = {
		["#inxiaofu"] = true,
	},
...
Room {
	...
	room_relative="松树林｜松树林----松树林----松树林｜空地松树林",
}
--[[Room {
	id = "huashan/shulinn",
	name = "松树林",
...
Room {
	...
	outdoor = "昆仑山",
	ways = {
		["out"] = "kunlun/shanlinn2",
	},
	objs = {
          ["冬虫草"] = "dongchong cao",
          ["黄芪"] = "huang qi",
     },
	 room_relative="葱岭谷∨云杉林葱岭谷",
}
Room {
	id = "kunlun/elang01",
	name = "东走廊",
...
Room {
	...
	outdoor = "昆仑山",
	ways = {
		["#walkBusy;out"] = "kunlun/shanlinn",
		["#walkBusy;up"] = "kunlun/kuhanlou2",
	},
}
Room {
	id = "kunlun/kuhanlou2",
	...
	outdoor = "昆仑山",
	ways = {
		["#walkBusy;down"] = "kunlun/kuhanlou1",
		["#walkBusy;up"] = "kunlun/kuhanlou3",
	},
}
Room {
	id = "kunlun/kuhanlou3",
	...
	outdoor = "昆仑山",
	ways = {
		["#walkBusy;down"] = "kunlun/kuhanlou2",
		["#walkBusy;out"] = "kunlun/houyuan",
	},
	room_relative="苦寒楼三层〓苦寒楼二层苦寒楼三层"
}
Room {
	id = "kunlun/lang",
	name = "走廊",
...
Room {
  ...
  ways = {
	...
	["south"] = "kunlun/shanlinn4",
	["west"] = "kunlun/shanlinn4",
  },
...
Room {
	...
	outdoor = "蝴蝶谷",
        ways = {
                ["south"] = "city/wroad3",
                --["#bohuacongcheck"] = "mingjiao/hdg/huacong1",
				["#bohuacongright"] = "mingjiao/hdg/huacong1",
...
Room {
    ...
    room_relative="练武场｜长廊----洪水旗----紫杉林洪水旗",
}
Room {
	id = "mingjiao/shuyuan",
	name = "书院",
...
Room {
	...
	room_relative="花丛中-----岸边-----花丛中｜花丛中岸边",
}
Room {
	id = "mtl/anbian1",
	name = "岸边",
...
Room {
	...
	objs = {
	 ...
	 room_relative="五指堂↑练功房-----广场-----瓜架｜山路广场",
}
Room {
	id = "tiezhang/hclu",
	name = "荒草路",
...
Room {
	...
	room_relative="玄岳门----山脚下----黄土路↘茶亭山脚下",
}
Room {
	id = "wudang/wulong",
	name = "五龙宫",
...
Room {
	...
	ways = {
		["east"] = "wudang/xiaolu3",
		["west"] = "wudang/xiaolu3",
		["south"] = "wudang/xiaolu3",
		["north"] = "wudang/xiaolu3",
		["#wdYm"] = "wudang/yuanmen",
		["#wdYmout"] = "wudang/xiaolu1",
	},
	lengths = {
		["#wdYm"] = 10,
	},
	room_relative="小径｜小径-----小径-----小径｜小径小径",
}
...
Room {
	...
	lengths = {
		...
		["n"] = "if score.master and score.master=='张三丰' then return false else return 1 end",
	},
	objs = {
          ["殷梨亭"] = "yin liting",
           },
...
Room {
	...
	ways = {
		["out"] = "xingxiu/shanshi",
		--["zuan"] = "xingxiu/cave2",
		["#xingxiuzuanenter"] = "xingxiu/cave2",
	},
	objs = {
          ["巨蟒"] = "ju mang",
          ["毒蛇"] = "du she",
	...
	blocks = {
		["#xingxiuzuanenter"] = {
			{id = "ju mang", exp = 60000},
			{id = "du she", exp = 60000},
		},
	},
	
	lengths = {
		--["zuan"] = "if Bag['火折'] then return 1 else return false end",
		["#xingxiuzuanenter"] = "if Bag['火折'] then return 1 else return false end",
	},
...
Room {
	...
	blocks = {
		["#xingxiuzuanenter"] = {
			{id = "piaoran zi", exp = 60000},
		},
	},
	
	lengths = {
		--["zuan out"] = "if Bag['火折'] then return 1 else return false end",
		["#xingxiuzuanenter"] = "if Bag['火折'] then return 1 else return false end",
	},
	room_relative="山洞山洞",
...
Room {
	...
	ways = {
        ...
        lengths = {
	    ["south"] = "if job.name and job.name=='wudang' or job.name=='huashan' or job.name=='xueshan' or job.name=='dolost' then return false else return 1 end",
        },
        nolooks = {
                ["south"] = true,
        },
		room_relative="丝绸之路丝绸之路↖｜万宝斋--吐谷浑伏俟城--沙洲｜南疆沙漠吐谷浑伏俟城",
}
Room {
	id = "xingxiu/silk9",
	name = "丝绸之路",
...
Room {
	...
	lengths = {
		...
		--["#yilicheckwd"] = "if MidNight[locl.time] and (job.name=='wudang' or job.name=='huashan') then return false else return 1 end",
		["#yilicheckwd"] = "if yili_midnigth_judge() then return false else return 1 end",
		--["north"] = "if yili_midnigth_judge() then return false else return 1 end",
	},
...
Room {
	...
	room_relative="街道｜猛虎营-----街道-----招财大车店｜明霞门街道",
}
Room {
	id = "xueshan/jiedao2",
	name = "街道",
...
Room {
	...
	room_relative="湖边｜湖边小路---湖边小路---佛照门湖边小路",
}
Room {
	id = "xueshan/xiekemen",
	name = "谢客亭",
