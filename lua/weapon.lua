...
--[[       if hp.neili<hp.neili_max*0.5 and cbb_cur>0 then
	   ...
	   for p in pairs(Bag) do
	   if Bag[p].kind and Bag[p].kind==skillEnable[perform.skill] and perform.skill~="wushang-dali" then
	   if not weapon.first or weapon.first~=p then
		  exe('wield '.. Bag[p].fullid)
	   end
	   end
		  if Bag[p].kind and Bag[p].kind=="staff" and perform.skill=="wushang-dali" then
		     if not weapon.first or weapon.first~=p then
		        exe('wield '.. Bag[p].fullid)
			 end
		  end
   ...
   print('weapon_unwield.......')
    for p in pairs(Bag) do
		...
		--[[if shenqi_id ~= nil then 
		  exe('unwield '..shenqi_id..' jian')
		end ]]--
		exe('unwield fengyun sword')
...
weaponUcheck=function()
	...
	exe('alias action checkUweapon')
	create_timer_s('checkUweapon_timer',1.0,'checkUweapon_test')
	
end

function checkUweapon_test()
   exe('alias action checkUweapon')
end 
...
weaponUdone=function()
	DeleteTimer("checkUweapon_timer")
...
weaponRepairDo=function()
	...
	create_trigger_t('repair5','^(> )*你带的零钱不够了！你需要(\\D*)锭黄金','','weaponRepairGold')
	...
	wait.make(function()
	     wait.time(1)
         exe('unwield xiao;unwield sword')
		 wait.time(1)
         exe('unwield sword 2')
		 wait.time(1)
...
function weaponRepairGold(n,l,w)
    print('weaponRepairGold...........')
	local repaire_gold = trans(w[2]) + 1
    EnableTriggerGroup("repair",false)
	EnableTimer("repair",false)
	exe('n;#3w;#3n;w;qu '..repaire_gold..' gold;e;#3s;#3e;s')
...
armorUcheck=function()
		  ...
		  create_timer_s('walkWait4',1.0,'armorUcheck_timer')
end

function armorUcheck_timer()
    exe('alias action checkUarmor')
end
...
function armorRepairGold(n,l,w)
         print('armorRepairGold.............')
         local repaire_gold = trans(w[2]) + 1
		 print('repaire_gold='..repaire_gold)
         EnableTriggerGroup("repair",false)
	     EnableTimer("repair",false)
	     exe('e;#3s;w;qu '..repair_gold..' gold;e;#3n;w')
...
armorRepairDo=function()
		 ...
		 create_trigger_t('repair5','^(> )*你带的零钱不够了！你需要(\\D*)锭黄金','','armorRepairGold')
...
function weapondrop()
        for p in pairs(Bag) do
                for q in pairs(ItemDropId) do
                        if p == q then
                                exe('unwield '..ItemDropId[q])
                                exe('drop '..ItemDropId[q])
                        end
                end
        end
end
