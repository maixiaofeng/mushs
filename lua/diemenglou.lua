...
function dmlTriggers()    
	...
	---pumkmai 添加,先准备好
	print('dmlTriggers...................')
	exe('jifa all;bei none;jifa cuff taizu-quan;bei cuff;jifa parry taizu-quan;jifa sword yuxiao-jian;jiali 1')
	exe('unwield taiji sword;unwield wuji xiao;wield fengyun sword;uweapon shape fengyun cuff')
...
function dmlHeal()
	...
	create_timer_s('walkWait4',3.0,'dmlHeal1')
end
function dmlHeal1()
	exe('ask xue muhua about 疗伤')
end
...
function checkTop(func)
	...
	create_trigger_t('checkTop1',"^(> )*(你现在已经组合|你已准备有一种技能了|你至少不会这两种拳脚技能的其中之一|你现在已经准备好特殊技能了)",'','beiOk')
...
function dmlInfo()
    ...
    if tonumber(GetVariable('dmlMode')) == 1 then
	   for i = 1,table.getn(_target) do
                   if _target[i].lvl <= _t1  and _target[i].rank < userRank then
                          DeleteTimer('dmlWait')
...
function dmlAsk()
	...
	--exe('jifa all;jifa parry yuxiao-jian;jiali 1')
	exe('jifa all;bei none;jifa cuff taizu-quan;bei cuff;jifa sword yuxiao-jian;jifa parry yuxiao-jian;jiali 1')
	
	if shenqi_id ~= nil then
	  exe('unwield '..shenqi_id..' jian;unwield fengyun sword;unwield shentong xiao;unwield taiji sword;wield fengyun sword')
	else
	  exe('unwield fengyun sword;unwield wuji xiao;unwield taiji sword;wield fengyun sword;uweapon shape fengyun cuff')
	end 
	
	
	---exe('bei none;bei hand')
...
function dmlDream()
	...
	if dmlFightCnt < 5 then
	   print('dml fight begin................................')
	   --exe('unwield shentong xiao;unwield taiji sword;wield fengyun sword')
	   --exe('jifa all;jiali 1;jifa parry yuxiao-jian')
       if l_cnt > 0 then
	      if table.getn(target) >= 5 then
...
function dmlfightcheck()
    print('dmlfightcheck...........')
    DeleteTriggerGroup('dmlfightck')
    create_trigger_t('dmlfightck1',"^·合气度·\\s*\\-?\\s*(\\d*)",'','dml_heqi_check')
    create_trigger_t('dmlfightck2',"你长啸一声，身形加快，施展出桃花绝技「星河千帆舞」，不时发出弹指神通指风！",'','dml_fight_buff')    ----改为自己buff或小pfm成功的描述，aoxue
    SetTriggerOption("dmlfightck1","group","dmfightck")
    SetTriggerOption("dmlfightck2","group","dmfightck")
    EnableTriggerGroup("dmlfightck",true)
end
dmlfightbuff=0

function dml_fight_buff()
        dmlfightbuff=1
end

function dml_heqi_check(n,l,w)
    print('dml_heqi_check...........')
    local dml_heqi = tonumber(w[1])
        if dml_heqi >= 200 and dmlfightbuff == 0 then
                exe('perform cuff.fumo')           ------自定义dml的pfmbuff或小pfm武功，aoxue
        elseif dml_heqi >= 160 then
                exe('jiali max;perform strike.chaodu')            ------自定义dml的pfm伤害武功，aoxue
        end
end
...
function dmlOver()
    if dmlFightCnt == 5 then
	   ...
	   local dml_cur_date = tonumber(os.date("%Y%m%d"))
	   SetVariable('dml_cur_date',dml_cur_date)
...
------pk攒合气设置--------
    --- pumkmai注释无需再判断这个pkheqi变量了
		--if pkheqi==1 and job.name=='diemenglou' then
