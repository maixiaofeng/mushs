...
function locate_trigger()
    ...
    create_trigger_t('locate6',"^(> )*现在是\\D*年\\D*月\\D*日(\\D*)时(\\D*)。$",'','local_time')
...
local_exit=function(n,l,w)
	...
	if string.find(locl.room, "江边") or string.find(locl.room, "渡口") or string.find(locl.room, "长江") then
        local l_c = 0
...
locate=function(thread)
    ...
    wait.make(function()
        ...
        while locate_exit ~= 1 do
		    locate_trigger()
			localget=0
            exe('alias action 正在定位;id here;set look;l;time')
            local l, w = wait.regexp('^(> )*你把 "action" 设定为 "正在定位" 成功完成。$',1)
            if l ~= nil then 
				local_start()
				local la, wa = wait.regexp('^( )*这里(看得清的|明显的|唯一的|看得见的唯一)出口是(\\D*)。$',1)
				if la ~= nil then
                    ll,ww = la,wa				
                    break					
				end
...
fastLocate=function(thread)
    ...
    wait.make(function()
        ...
        while locate_exit ~= 1 do
		    locate_trigger()
			localget=0
            exe('alias action 正在定位;set look;l')
            --exe('set look;l')
            local l, w = wait.regexp('^(> )*你把 "action" 设定为 "正在定位" 成功完成。$',1)
            if l ~= nil then 
			    --print('locate start...')
...
function path_consider()
    ...
    if sour.id == nil then
       ...
       sour.room=locl.room
       sour.area=locl.area
       if sour.area == '不知道哪里' and locl.room_relation ~= nil then 
	      print('test!!!!!!!!!!!!!')
    ...
    if sour.id ~= nil then
       ...
       if sour.room=="观星台" then
	    ...
	    if locl.room_relation=='九老洞九老洞' or locl.room_relation=="不知道哪里九老洞 不知道哪里 九老洞" or (string.find(locl.room_relation,'九老洞九老洞') and locl.area=='不知道哪里') then
		    print('九老洞 test ..........')
             exe('drop fire;leave;leave;leave;leave;leave;leave;out;ne;ed;ne;ed')        
             return checkWait(goContinue,0.3)
        end
		
		if locl.room_relation=='山洞山洞' and locl.area=='星宿海' and locl.room=='山洞' then 
            ...
            for i=1,table.getn(sour.rooms) do
				...
				--print('map.rooms[sour.rooms[i]].room_relative:'..map.rooms[sour.rooms[i]].room_relative)
			        if map.rooms[sour.rooms[i]].room_relative == nil then 
					    chats_locate('定位系统：重名房间但是没有room_relative 【'.. sour.rooms[i] ..'】资料，需要关注！','red')
...
function go_confirm()
    ...
    if (locl.room==dest.room and locl.area ~='不知道哪里' and locl.area==dest.area ) or (locl.room==dest.room and locl.area =='不知道哪里') or (locl.room==dest.room and string.find(dest.area,"/")) or flag.go==0 then
...
find_nobody=function()
	...
	--[[
	if job.name=='wudang' and (string.find(job.where,'襄阳城猪肉店') or string.find(job.where,'襄阳城北城巷') or string.find(job.where,'襄阳城鲜鱼店') or string.find(job.where,'襄阳城北城巷') or string.find(job.where,'嵩山少林心禅坪') or string.find(job.where,'嵩山少林罗汉堂')) then 
...
function searchPre()
  ...
  --[[if job.name and (job.name=="clb" or job.name=='tdh' or job.name=='tmonk') and flag.times==1 then
   ...
   if job.name=='zhuacaishen' or job.name=='dolost' then
	...
	if p_room=='南城门' and p_dest=='伊犁城' and (job.name=='wudang' or job.name=='huashan' or job.name=='xueshan') then
	    if l_distance<3 then l_distance=3 end
	end 
	
	if p_room=='西大街' and p_dest=='扬州城' and job.ori_where ~= nil then
	    l_distance=3 
	end 
	
	if job.name=='huashan' and flag.times==1 and job.area=='华山' and job.room=='侧廊' then
	    l_distance=0
	end
	...
	local rooms = {}
	--flag.searchRooms = {{'烈火旗','明教',1},{'厚土旗','明教',1},{'洪水旗','明教',1},{'锐金旗','明教',1}}
	...
	elseif (job.name=='wudang' or job.name=='huashan') and job.ori_where~=nil and job.where=="扬州城西大街" then
	...
	if (job.name=='wudang' or job.name=='huashan')  and job.where=='伊犁城南城门' then
	...
	if (job.name=='wudang' or job.name=='huashan')  and job.where=='扬州城西大街' then
	   flag.search_yili_center=1
	   if locl.room_relation=='西门----西大街----西大街西大街' then 
	          flag.search_yili_center=0
	...
	---hs 任务如果是伊犁城内则直接处理失败
	--[[if job.name=='huashan' and ori_job_where ~= nil and job.where=='伊犁城南城门' then
	    if locl.room_relation=='南城门｜伊犁河南城门' then 
		    rooms={}
		end
	end
...
function searchStart()
	...
	if string.find(path,'#') or job.name~='huashan' or (job.name=='huashan' and flag.hs_find_lost==1) then
		...
		if tmpnum > 50 or knock_gate==1 or #road.rooms==0 then 
		    --print('搜索超过限制步数了,或者road.room已没有了...')
			print('**执行:'..walk_path)
			Send('alias walkpath '..walk_path)
			exe('walkpath')
			walk_path = ''
...
hscaidi=function()
    ...
    create_trigger_t('hscaidi1','^(> )*你乱走了一通，居然发现自己走回了原地。','','hscaidi_out1')
	create_trigger_t('hscaidi2','^(> )*这个方向没有出路。$','','hscaidi_out2')
...
function PoutShangguan()
	...
	create_trigger_t('tiezhangsg5','^什么?$','','ShangguanWalkError')
...
function PoutShangguan_confirm()
    ...
    if locl.room=='无名峰' then 
	    EnableTriggerGroup("tiezhangsg",true)
		SetSpeedWalkDelay(30)
	    exe("move bei")
	else
	    --return go(PoutShangguan_confirm,'铁掌山','无名峰')
		return go(road.act)
	end  
end
...
function GoShangguanQiuError()
	...
	wait.make(function()
	    wait.time(1)
	    exe("se;ask qianzhang about 闹鬼")
		wait.time(1)
		exe("nw;w;ask qianzhang about 闹鬼")
		wait.time(1)
		exe("w;ask qianzhang about 闹鬼")
		wait.time(1)
		exe("e;nu;ask qianzhang about 闹鬼")
		wait.time(1)
		exe("sd;s;ask qianzhang about 闹鬼;n;e")
	end)
...
function toSld()
	...
	wait.make(function()
        wait.time(1)
	    if locl.room=='海滩' and locl.area=='神龙岛' then
	      weapon_unwield()
		  wait.time(1)
          sld_weaponWieldCut()
		  wait.time(1)
...
function goxtjck()
   ...
   if locl.room_relation=='树林｜树林-----树林-----树林｜树林树林'  then 
        return goXtjGo()
   end 
   
   if locl.room_relation=='树林｜树林-----树林-----树林｜山路树林'  then 
       return goXtjGo_Ex()
   end  
...
function outxtjck()
   ...
   if flag.find==1 then return end
    wait.make(function() 
		...
		if locl.room_relation=='树林｜树林-----树林-----树林｜山路树林' or locl.room_relation=='山路｜山路↘山间空地山路' then
            exe('s')
            return outXtjOver()
        end
		if locl.room_relation=='树林｜树林-----树林-----树林｜树林树林' then 
...
function xingxiuzuanenter()
	...
	create_trigger_t('xingxiuzuanenter1',"^(> )*你想往哪儿钻",'','xingxiuzuanenter_sucess')
	--create_trigger_t('xingxiuzuanenter1',"^(> )*你点燃了火折，发现西面石壁上有一道裂缝，似乎可以钻",'','xingxiuzuanenter_sucess')
...
function xingxiushandong_confirm()
    ...
    if locl.room=='山石' and locl.room_relation=='山石←海边荒路山石'  then 
	    EnableTriggerGroup("xxsdAsk",true)
	    EnableTriggerGroup("wipe",false)
	    exe("ask shihou zi about fqyy")
	else
...
function xingxinshandong_new()
   ...
   create_trigger_t('kill_shihouzi1','^(> )*狮吼子「啪」的一声倒在地上，挣扎着抽动了几下就死了','','xingxinshandong_shihouzi_kill_over')
   ...
   wait.make(function()    
	   ...
	   while true do
	       wait.time(1)
	       exe("ask shihou zi about fqyy")
		   local l,w = wait.regexp('^(> )*(你向狮吼子打听有关『fqyy』的消息|这里没有这个人。)',1)
		   if l ~= nil then
...
function xingxinshandong_shihouzi_kill()
    ...
    wait.make(function()
		while true do
	       wait.time(1)
	       exe("kill shihou zi")
		   l, w = wait.regexp('^(> )*(你对着狮吼子大喝一声|这里没有这个人)',1)
		   if l ~= nil then
...
function xxsdover()
	...
	wait.make(function()
	   while true do
	       exe("enter cave")
		   local l,w  = wait.regexp('(> )*你小心翼翼地往山洞里探了探。$',1)
...
function xiaofu_check_enter()
    wait.make(function()
	    while true do
		    exe('north')
			local l,w = wait.regexp("^(> )*(张妈一手把你抓住|萧府大厅\\s*-)",1)
			if l~=nil then 
			    if string.find(l,"张妈一手把你抓住") then 
...
function yili_midnigth_judge()
    if locl.area=='伊犁城' and locl.room=='南城门' then 
	   return false
	end
	
	if locl.area=='扬州城' and locl.room=='西大街' then 
	   return false
	end
    ...
    if locl.time=='酉' or locl.time=='戌' or locl.time=='亥' or  locl.time=='子' then
	   return true
	end
    
	if locl.time=='丑' and locl.time_sk=='正' then
       return true
    end	
	
	if locl.time=='申' and locl.time_sk=='三刻' then
       return true
    end	
...
function bohuacongright()
    wait.make(function()
	    ...
	    fastLocate(coroutine.running())
		coroutine.yield()
		if locl.room_relation=='山壁｜土路山壁'  or  locl.room_relation=='山壁〉花丛中｜土路山壁' then
...
function bohuacongcheck()
    wait.make(function()
	    ...
	    fastLocate(coroutine.running())
		coroutine.yield()
		if locl.room_relation=='山壁｜土路山壁' then
...
function gosxh_consider()
	...
	elseif locl.room_relation=='西门----西大街----西大街西大街' then
	   if job.name=='wudang' and job.ori_where~=nil and flag.search_yili_center==1 then 
			return wudangYiliFindFail()
	   end
	   return sxh_wait()
...
function mtlcsg_check_up()
    wait.make(function()
		    while true do 
		        exe('up')
			    local l,w = wait.regexp("^(> )*(王夫人哼了一声：“楼上是我曼佗罗山庄的藏书阁，你也想上|藏书阁\\s*-\\s*)",1)
			    if l~=nil then 
			        if string.find(l,"王夫人哼了一声") then 
...
function mtlcsg_check_wang()
    ...
    create_trigger_t('mlt_cangshuge1',"^(> )*王夫人神志迷糊，脚下一个不稳，倒在地上昏了过去",'','mtlcsg_over_a')
	create_trigger_t('mlt_cangshuge2',"^(> )*王夫人「啪」的一声倒在地上，挣扎着抽动了几下就死了",'','mtlcsg_over_c')
...
function mtlcsg_over_a()
	wait.make(function()
		while true do
		    exe('get wang')
			local l,w = wait.regexp("^(> )*(你将王夫人扶了起来背在背上|你附近没有这样东西)",1)
			if l~= nil then 
