...
job.list={}
sxjob={}
sxjob.ide='令狐冲|黄令天|凌震天|中年乞丐|青年乞丐|灰衣帮众|五毒教弟子|慧真尊者|知客道长|女弟子|家丁|游客|挑夫|巨木旗教众|锐金旗教众|洪水旗教众|厚土旗教众|烈火旗教众|出尘子|李捕头|值勤兵|樊一翁|张巡捕|护法使者|夏雪宜|蒙面人|黑衣帮众|土匪|马贼|大衙役'
sxjob.cancel=''
sxjob.skillslist=''
sxjob.killer1='杀手一'
sxjob.killer1='杀手二'
...
pfm_heqi_count = tonumber(GetVariable('pfm_heqi_count'))
pfm_bieqidz = tonumber(GetVariable('pfm_bieqidz'))
--根据战斗情况调整憋气合气次数
pfm_heqi_count_add=0
pfm_use_kuangfeng=0
--pfm 模式
...
--    create_trigger_t('fight4',"^(> )*看起来(\\D*)想杀死你！$",'','fightConsider')
	...
	create_trigger_t('fight19',"^(> )*你一声清啸，拳脚齐施，使出桃花岛「狂风绝技」迅捷无伦地攻向",'','StartKuangfeng')
	create_trigger_t('fight20',"^(> )*你一套狂风绝技施展完毕，身形慢了下来",'','StopKuangfeng')
	
	---create_trigger_t('fight21',"^(> )*你猛然伏地，使出断门刀「断」字决，顿时一片白光直向郎封滚去！",'','StartKuangfeng')
	
	create_trigger_t('fight22',"^(> )*你的武技与对手悬殊太大，无法造成有效伤害",'','UseKuangfeng')
	--create_trigger_t('fight23',"^(\\D*)(深深吸了几口气，脸色看起来好多了)",'','fight_check')
	...
	--SetTriggerOption("fight21","group","fight")
	SetTriggerOption("fight22","group","fight")
	--SetTriggerOption("fight23","group","fight")
...
function UseKuangfeng()
   print('UseKuangfeng......')
    -- pfm_use_kuangfeng = 1
end

function StartKuangfeng()
   print('StartKuangfeng....')
   exe('jiali max')
end 

function StopKuangfeng()
   print('StopKuangfeng....')
   exe('jiali 1')
end
...
function hp_heqi_check(n,l,w)
 heqi = tonumber(w[1])
	...
	if score.xiangyun and string.find(pfm_xiangyun,score.xiangyun) then
        pfm_bieqidz = tonumber(GetVariable('pfm_bieqidz_si'))
    else
	     pfm_bieqidz = tonumber(GetVariable('pfm_bieqidz'))
	end 
 
--[[ 
 if heqi_count==0 and heqi < 30 and (job.name == 'huashan' or job.name=='wudang' or job.name=='xueshan')  then
    --heqi_count = -1
	print('jiaosha_kill...'..jiaosha_kill)
	print('job.name...'..job.name)
	if jiaosha_kill==0 and (job.name == 'huashan' or job.name=='wudang') and job.id~=nil then
	
	   print('合气太低..判断是没有下kill')
 ...
 --if heqi_count == -1 and  heqi > 160 and (job.name == 'huashan' or job.name=='wudang')  then 
 --   heqi_count = 1
	--kezhiwugong()
...
-- end 
  ...
  --- mode 0 不判断npc skill ，
  --- mode 1 判断npc skill，但是不判断相运
  --- mode 2 判断npc skill  和相运
  
  if heqi_count == 0 and  job.name == 'wudang' then 
  
  if pfm_bieqie_mode == 1 then
    --- wudang job 看到难打的直接憋气
   if string.find(pfm_yanchi,sxjob.skills) then 
     print('wdjob ..mode 1')
     heqi_count = -1
	 exe('set wimpycmd hp')
	 kezhiwugong()
  ...
  elseif  pfm_bieqie_mode == 2 then
     --- wudang job 看到难打的 ,和相运不好，直接憋气
     if string.find(pfm_yanchi,sxjob.skills) and string.find(pfm_xiangyun,score.xiangyun) then 
	   print('wdjob ..mode 2')
       heqi_count = -1
	   exe('set wimpycmd hp')
	   kezhiwugong()
  ...
  --[[if heqi_count == 0 and  job.name == 'huashan' and string.find(pfm_xiangyun,score.xiangyun) then 
    heqi_count = -1
	exe('set wimpycmd hp')
	kezhiwugong()
	kezhiwugongAddTarget(job.target,job.id)
 ...
 if heqi_count >=0 then 
   heqi_count = heqi_count + 1
 end
 
 ---print('pfm_heqi_count_add='..pfm_heqi_count_add)
  
 -- 10次还没有解决战斗，说明pfm不行
 if heqi_count > (pfm_heqi_count + pfm_heqi_count_add) and  (job.name == 'huashan' or job.name=='wudang') then 
    heqi_count = -1
	exe('set wimpycmd hp')
	kezhiwugong()
	kezhiwugongAddTarget(job.target,job.id)
 ...
 if heqi_count > (pfm_heqi_count + pfm_heqi_count_add) and  (job.name == 'xueshan') then 
    heqi_count = -1
	exe('set wimpycmd hp')
	kezhiwugong()
 ...
 if pfm_use_kuangfeng == 1  then 
     pfm_bieqidz = tonumber(GetVariable('pfm_bieqidz_si'))
 end 
 
 print('pfm_bieqidz='..pfm_bieqidz)
 if heqi_count==-1 and heqi > pfm_bieqidz and  (job.name == 'huashan' or job.name=='wudang') then
    heqi_count = -2
	kezhiwugong()
	kezhiwugongAddTarget(job.target,job.id)
 end 
  ...
  if heqi_count==-1 and heqi > pfm_bieqidz and job.name=='xueshan' then
    heqi_count = -2
	kezhiwugong()
	kezhiwugongAddTarget(gdcname,guard_id)
 end 
  ...
  exe('eat '.. drug.neili)
	...
	if pkheqi==1 and (job.name=='pk' or job.name=='diemenglou' or job.name == 'guanfu') then
           if heqi > tonumber(GetVariable("heqi_number")) then
				...
				print('npc_value.....11111........'..npc_val)
				if npc_val=='慢' or npc_val=='柔' or npc_val=='奇' then 
				    ---exe('unwield fengyun sword;wield shentong xiao;jiali max;perform sword.yuxiao;perform strike.pikong;jiali 1')
					exe('jiali max;perform parry.yuxiao;perform strike.pikong;jiali 1')
				else 
				    ---exe('unwield shentong xiao;wield fengyun sword;jiali max;perform sword.yuxiao;perform strike.pikong;jiali 1')
					exe('jiali max;perform parry.yuxiao;perform strike.pikong;jiali 1')
                end
           end
    end     
...
function fight_check(n,l,w,styles)
 ...
 if w[1]=='你' then
  ...
  if string.find(l,"看起来气血充盈，并没有受伤") then
	...
	--[[if l=="深深吸了几口气，脸色看起来好多了" then
	   print('深深吸了几口气，脸色看起来好多了.........................')
	   pfm_heqi_count_add = pfm_heqi_count_add - 4
	end]]--
	
	if heqi_count > 0 then 
	    if string.find(l,"似乎有些疲惫，但是仍然十分有活力") then
	      pfm_heqi_count_add = pfm_heqi_count_add - 0.3
	    end
	
	   if string.find(l,"动作似乎开始有点不太灵光，但是仍然有条不紊") then
	     pfm_heqi_count_add = pfm_heqi_count_add + 0.1
	   end
	  ...
	  if string.find(l,"气喘嘘嘘，看起来状况并不太好") then
	    pfm_heqi_count_add = pfm_heqi_count_add + 0.2
	  end
	
	  if string.find(l,"似乎十分疲惫，看来需要好好休息了") then
	     pfm_heqi_count_add = pfm_heqi_count_add + 0.5
	  end
	  ...
	  if string.find(l,"受了相当重的伤，只怕会有生命危险") then
	   pfm_heqi_count_add = pfm_heqi_count_add + 1.5
	  end
	  if string.find(l,"伤重之下已经难以支撑，眼看就要倒在地上") then
	    pfm_heqi_count_add = pfm_heqi_count_add + 1.5
	  end
	  ...
	  if string.find(l,"摇头晃脑、歪歪斜斜地站都站不稳，眼看就要倒在地上") then
	   pfm_heqi_count_add = pfm_heqi_count_add + 2
	  end
	  if string.find(l,"已经陷入半昏迷状态，随时都可能摔倒晕去") then
	   pfm_heqi_count_add = pfm_heqi_count_add + 2
	  end
...
function fightHpCheck()
	...
	if hp.neili<hp.neili_max*0.2 and nxw_cur>0 then
       exe('eat '.. drug.neili)
    end
	if hp.neili<hp.neili_max*0.5 and cbw_cur>0 then
       exe('eat '.. drug.neili2)
    end
  if hp.neili<hp.neili_max*0.4 and hqd_cur>0 then
  exe('eat '.. drug.neili3)
    end
	    if hp.jingli<hp.jingli_max*0.5 or hp.jingli<500 then
       exe('yun jingli')
...
function faint_check()
	...
	wait.make(function()
	    wait_busy()
		exe('drink jiudai;drink jiudai;drink jiudai')
		wait_busy()
		exe('drink jiudai;drink jiudai;drink jiudai')
	    job.killer = {}
	    tmp = {}
...
function job_exp_trigger()
    ...
    create_trigger_t('job_exp12',"^(> )*你获得了(\\D*)点经验，(\\D*)点潜能，(\\D*)点\\D*神。$",'','job_exp_huashan','cyan')
	...
	create_trigger_t('job_exp19',"^(> )*宋远桥为你在钱庄里存入(\\D*)锭黄金。",'','job_gold_wudang')
	create_trigger_t('job_exp20',"^(> )*宝象为你在钱庄里存入(\\D*)锭黄金。",'','job_gold_xueshan')
	create_trigger_t('job_exp21',"^(> )*洪七公为你在钱庄里存入(\\D*)锭黄金。",'','job_gold_h7gzc')
	...
	SetTriggerOption("job_exp20","group","job_exp")
	SetTriggerOption("job_exp21","group","job_exp")
...
function job_exp_xueshan(n,l,w)
   ...
   xstongji_cgl=string.format("%0.2f",100-xstongji_fangqi/xstongji_finish*100) 
   messageShow('雪山任务奖励：经验:【'..w[2]..'】点，潜能:【'..w[3]..'】点！','gold')
   
   messageShow('雪山任务统计，本次用时:【'..job.time.over..'】秒，平均战斗时间【'..xstongji_pingjun..'】秒,雪山领取任务【'..xstongji_cs..'】次,成功【'..xstongji_finish..'】次，失败【'..xstongji_fangqi..'】次，成功率【'..xstongji_cgl..'%】,黄金【'..tjxsgold..'】两。','gold')
end
function job_gold_wudang(n,l,w)
      local x
      x=trans(w[2])
	  tjwdgold=tjwdgold+x
end
function job_gold_xueshan(n,l,w)
      local x
      x=trans(w[2])
	  tjxsgold=tjxsgold+x
end
function job_gold_h7gzc(n,l,w)
      local x
      x=trans(w[2])
	  tjzcgold=tjzcgold+x
	  messageShow('洪七公做菜奖励：闪闪的金子:【'..tjzcgold..'】锭！','gold')
end
function job_exp_wudang(n,l,w)
    messageShow('武当任务奖励：经验:【'..w[2]..'】点，潜能:【'..w[3]..'】点！','gold')
	wudangjobok=1
	
	job.wdtime=os.time()+40
	job.time.e=os.time()
	job.time.over=job.time.e-job.time.b
	wdtongji_cgl=string.format("%0.2f",100-wdtongji_fangqi/wdtongji_finish*100) 
       messageShowT('武当任务统计,本次用时:【'..job.time.over..'】秒，平均战斗时间【'..wdtongji_pingjun..'】秒,武当领取任务【'..wdtongji_cs..'】次,成功【'..wdtongji_finish..'】次，失败【'..wdtongji_fangqi..'】次，成功率【'..wdtongji_cgl..'%】,黄金【'..tjwdgold..'】两。','gold')
		dis_all()
	return checkWait(wudangFinish,1)
end
function job_exp_huashan(n,l,w)
    hstongji_cgl=string.format("%0.2f",100-hstongji_fangqi/hstongji_finish*100) 
	messageShow('华山任务奖励:经验:【'..w[2]..'】点，潜能:【'..w[3]..'】点!')
    messageShow('华山任务统计,本次用时:【'..job.time.over..'】秒,华山领取任务【'..hstongji_lq..'】次,成功【'..hstongji_finish..'】次，失败【'..hstongji_fangqi..'】次，成功率【'..hstongji_cgl..'%】。','aqua')
end
...
function job_exp_tongji(p_cmd)
	...
	--[[if flag.log and flag.log=="yes" then
	  tjsld=tosld+wdtosld+xstosld
	  tjcaidi=tocaidi
	  tjtomr=tomr+wdtomr+xstomr
	  tjyzw=toyzw+wdtoyzw+xstoyzw
	  tjmtl=tomtl+wdtomtl+xstomtl
	  tjszgf=hstoszgf+wdtoszgf+xstoszgf
	  tjhssd=hstohssd+xstohssd+wdtohssd
	  tjgold=tjxsgold+tjwdgold+tjzcgold
	  tjtoxct=hstoxct+wdtoxct+xstoxct
       messageShowT('任务奖励统计：共运行【'.. l_hour ..'小时'.. l_min .. '分' .. l_sec ..'秒' ..'】，获得经验【'..l_exp..'】点，平均每小时【'..l_avg..'】点！','orange')
	   messageShow('任务地点总计：神龙岛【'..tjsld..'】次 ！菜地【'..tjcaidi..'】次 ！慕容【'..tjtomr..'】次 ！燕子坞【'..tjyzw..'】次 ！曼佗罗山庄【'..tjmtl..'】次 ！',"paleturquoise")
	   messageShow('任务不可到达地点统计：华山石洞【'..tjhssd..'】次 ！苏州闺房【'..tjszgf..'】次 ！心禅堂【'..tjtoxct..'】次 ！',"paleturquoise")
	   messageShow('任务黄金总计【'..tjgold..'】两!  在线兵器修理【'..bqxl..'】 次   在线护具修理【'..hujuxl..'】 次',"paleturquoise")  
    else
...
zhunbeineili=function(func,p_cmd)
   ...
   zbnl = tonumber(GetVariable("zbnl"))
   zbnl_si = tonumber(GetVariable("zbnl_si"))
   pfm_xiangyun = GetVariable('pfm_xiangyun')
   if zbnl==nil or (score.xiangyun ~=nil and string.find(pfm_xiangyun,score.xiangyun)) then
	    print('特殊理相打坐倍数...')
        zbnl = zbnl_si
    end		
	
	print('zbnl='..zbnl)
	--exe('unwield fengyun sword;unwield shentong xiao;wield taiji sword')
    ...
    if hp.neili>=hp.neili_max*zbnl then
      return check_bei(job.prepare)
    end
...
zhunbeineili_a=function()
   ...
   ---pumkmai 定制
		...
		wait_busy()
		  ...
		  wait_busy()
		  ---wait.time(1)
		  print('dazuo wield huifu weapon.....')
		  exe('unwield fengyun sword;unwield wuji xiao;wield taiji sword;uweapon shape fengyun sword')
		  wait_busy()
          wait.time(0.5)		  
		  exe('dazuo '..hp.dazuo)
	     end)
...
zhunbeineili_c=function()
   print('zhunbeineili_c..........')
   zbnl = tonumber(GetVariable("zbnl"))
   zbnl_si = tonumber(GetVariable("zbnl_si"))
   pfm_xiangyun = GetVariable('pfm_xiangyun')
	if zbnl==nil or string.find(pfm_xiangyun,score.xiangyun) then
	    print('特殊理相打坐倍数...')
        zbnl = zbnl_si
    end		
	print('zbnl='..zbnl)
...
function scrReloginLog()
    local filename = GetInfo (67) .. "logs\\" .. score.id .. '重新登录'..os.date("%Y%m%d_%H时%M分%S秒") .. ".log"
...
function jobtongjiLog()
    local filename = GetInfo (67) .. "logs\\" .. score.id .. '任务统计'..os.date("%Y%m%d_%H时%M分%S秒") .. ".log"
...
function lingwu_finish_log()
    local filename = GetInfo (67) .. "logs\\" .. score.id .. '领悟完成'..os.date("%Y%m%d_%H时%M分%S秒") .. ".log"
