...
function button_smyteam()
   ...
   WindowCreate(win2,0,0,140,300,6,16,ColourNameToRGB("black"))
  ...
  local _mousedown_newbie=mousedown_newbie()
   ...
   _G["at_mousedown_pkheqi"]=_mousedown_pkheqi
	WindowAddHotspot(win2, "btn_pkheqi",
   ...
   local _mousedown_dolostagain=mousedown_dolostagain()
   _G["at_mousedown_dolostagain"]=_mousedown_dolostagain
	WindowAddHotspot(win2, "btn_dolostagain",
                    0, 260,140, 280,   -- rectangle
                   "",   -- MouseOver
                   "",   -- CancelMouseOver
                   "at_mousedown_dolostagain",  -- MouseDown
                   "",   -- CancelMouseDown
                   "",   -- MouseUp
                   "左键点击开关",  -- tooltip text
                   cursor or 1, -- cursor
    ...
    local _mousedown_gotodp=mousedown_gotodp()
   _G["at_mousedown_gotodp"]=_mousedown_gotodp
	WindowAddHotspot(win2, "btn_gotodp",
                    0, 280,140, 300,   -- rectangle
                   "",   -- MouseOver
                   "",   -- CancelMouseOver
                   "at_mousedown_gotodp",  -- MouseDown
                   "",   -- CancelMouseDown
                   "",   -- MouseUp
                   "左键点击开关",  -- tooltip text
                   cursor or 1, -- cursor
	  ...
	  if pkheqi==1 then
		 switch_name13="PK憋合气-开"
	  else
		 switch_name13="PK憋合气-关"
	  end
	  
	    
	  if do_lost_again==1 then 
	      switch_name14="连续做LL--开"
	  else
	      switch_name14="连续做LL--关"
	  end 
	 ...
	 WindowText (win2, "f", switch_name13,0,240,140,260,ColourNameToRGB ("red"), false) -- not Unicode
	 
	 WindowText (win2, "f", switch_name14,0,260,140,280,ColourNameToRGB ("gold"), false) -- not Unicode
	 
	 WindowText (win2, "f", switch_name15,0,280,140,300,ColourNameToRGB ("gold"), false) -- not Unicode
...
function mousedown_pkheqi() --无法风20181218增加变量，pkheqi开关，控制pk是否憋合气。
	  return function (flags, hotspot_id)
...
function mousedown_dolostagain()
     return function (flags, hotspot_id)
	if hotspot_id=='btn_dolostagain' then
		if do_lost_again==0 then
			do_lost_again=1
		...
		SetVariable('do_lost_again',do_lost_again)
		...
		if flags==16 then
		  ...
		  WindowRectOp (win2, miniwin.rect_fill, 0,260,140,280,ColourNameToRGB("black"))  -- raised, filled, softer, flat 0x909090
		  WindowFont (win2, "f", "新宋体", 10, true, false, false, false)
		  WindowText (win2, "f", switch_name14,0,260,140,280,ColourNameToRGB ("gold"), false) -- not Unicode
		  WindowShow (win2,  true)  -- show it
		end
...
function mousedown_gotodp()
     return function (flags, hotspot_id)
	if hotspot_id=='btn_gotodp' then
		if go_dp==0 then
			go_dp=1
		...
		if flags==16 then
		  ...
		  WindowRectOp (win2, miniwin.rect_fill, 0,280,140,300,ColourNameToRGB("black"))  -- raised, filled, softer, flat 0x909090
		  WindowFont (win2, "f", "新宋体", 10, true, false, false, false)
		  WindowText (win2, "f", switch_name15,0,280,140,300,ColourNameToRGB ("gold"), false) -- not Unicode
		  WindowShow (win2,  true)  -- show it
		end
  ...
  WindowAddHotspot(win_bag, "bag",  
   ...
   WindowText (win_bag, "f1", 
               "共"..#_bag_ch.."件物品",   -- text
                ...
                ColourNameToRGB ("white"), -- colour
                false)              -- not Unicode
  if Bag["ENCB"] and Bag["ENCB"].value then               --deke
   WindowText (win_bag, "f1", 
