...
function songxin_trigger()
   ...
   --[[
    ...
    create_trigger_t('letteryichang1',"^(> )*你紧张起来，不由一摸衣袋，不好！信被偷了!",'','sx2over')
    create_trigger_t('letteryichang2',"^(> )*你看了看信，决定还是不去了。",'','sx2overok')
...
function job_songxin()
  ...
  --  end
    job.time.b=os.time()
    return check_busy(songxin_ask11)
 end
function songxin_ask11()
  wipe_kill=1
  exe('ask zhu wanli about job')
  create_timer_s('walkWait4',3.0,'songxin_ask_test')
 end
songxin_ask_test=function()
    exe('ask zhu wanli about job')       
end
...
function songxin_xin()
    ...
    sxkiller1=0
    sxkiller2=0
	return check_bei(songxin_test)
    
end
function songxin_test()
--exe('set po 掌;look letter')
   wait.make(function()
       ...
       while true do
             exe('look letter')
			 l,w = wait.regexp('^(> )*\\s*「(\\D*) (\\D*)」 亲启。',1)
			 if l~=nil then
			     break
			 end 
       end	   
	   songxin_consider(nil,l,w)
   end)

end
function songxin_consider(n,l,w)
    print('songxin_consider................')
    dis_all()
    nobusy=1
    --EnableTriggerGroup("songxin_letter",false)
	...
	if (job.target=="边防官兵" and job.where=="嘉峪关东城门") or 
	   (job.target=="蒙面人" and job.where=="扬州城大驿道") or 
	  --[[  (job.target=="烈火旗教众") or 
		(job.target=="厚土旗教众") or 
		(job.target=="锐金旗教众") or 
		(job.target=="洪水旗教众") or 
		...
		(job.target=="张三丰") or 
		(job.target=="夏雪宜") or 
	   --(job.target=="玄苦大师" and job.where=="嵩山少林") or 
	    string.find(job.where,"曼佗罗山庄") or 
		string.find(job.where,"燕子坞")  or 
		(job.target=="任飞燕" and job.where=="黄河流域黄河岸边")   then
		messageShow('送信任务：纯粹扯谈的送信:'..job.target..';'..job.where,'blue')
       return check_halt(songxin_fangqi)
    end
  ...
  -------------------- 
   ...
   -- if not job.area then
		...
		if not job.room  then
		   messageShow('送信任务：任务地点【'..job.where..'】为空，不可到达，任务放弃。','blue')
		   return check_halt(songxin_fangqi)
		end
		
		if not path_cal() then
		   messageShow('送信任务：任务地点【'..job.where..'】无法计算路径，不可到达，任务放弃。','blue')
		   return check_halt(songxin_fangqi)
		end
...
function songxin_find()
    ...
    --create_trigger_t('songxin_find1','^>*\\s*\\D*'..job.target..'\\((\\D*)\\)','','songxin_send')
	 create_trigger_t('songxin_find1','^>*\\s*\\D*'..job.target..'\\((\\D*)\\)','','songxin_target_find')
	...
	--pumkmai add
	---create_trigger_t('songxin_find6','^(> )*你把 "action" 设定为 "检查信是否还在" 成功完成。$','','songxin_check')
	...
	---pumkmai 增加
	SetTriggerOption("songxin_find1","sequence",1)
    EnableTriggerGroup("songxin_find",false)
	--sx_finish=0
end


function songxin_target_find(n,l,w)
    print('songxin_target_find.................................')
    EnableTrigger("songxin_find1",false)
	sxjob.id=string.lower(w[1])
    if flag.wait==0 then
       flag.wait=1
       --sxjob.cnt=1
	   --checkNext(songxin_next1)
    end
	
	DeleteTriggerGroup("xin_check")
    create_trigger_t('xin_check1','^\\s*一封信件\\(Letter\\)','','mail_check')
    SetTriggerOption("xin_check1","group","xin_check")
	mail_cnt = 0
	...
	wait.make(function()
        while true do
             exe('follow '..sxjob.id)
             exe('i') 
             exe('alias action 检查信是否还在')	
             local l, w = wait.regexp('^(> )*你把 "action" 设定为 "检查信是否还在" 成功完成。$',1)
             if l ~= nil then break end
        end
        if mail_cnt==1 then
	       mail_cnt=0
	       EnableTriggerGroup("xin_check",false)
           return check_halt(songxin_send_act_new)
        else
           return check_heal()
        end
    end)
...
function songxin_send_act_new()
    print('songxin_send_act_new.................................')
   ...
   locate_finish=0
	...
	wait.make(function()
	    while sx_fail==0 do
	        while true do
	            wait.time(1)
				exe('halt;songxin '..sxjob.id..' '..sx_cnt)
                local l, w = wait.regexp('^>*\\s*(你擦了一把额头的汗，从怀中掏出信交给|这封信不是送给这个人的。|看清楚点，那是活人吗？！|你要送给谁|\\D*道：这封信不是给我的，|\\D*道：你看清楚，)',1)  
				if l ~= nil then
				  print('sx 指令后触发...............')
					...
					--[[
					if string.find(l,'这封信不是送给这个人的。') or string.find(l,'看清楚点，那是活人') or string.find(l,'这封信不是给我的') or string.find(l,'你看清楚') then 
					    break
					end ]]--
...
function songxin_next1()
    print('songxin_next1.......')
    DeleteTriggerGroup("xin_check")
    create_trigger_t('xin_check1','^\\s*一封信件\\(Letter\\)','','mail_check')
	--create_trigger_t('xin_check2','^(> )*你把 "action" 设定为 "检查信是否还在" 成功完成。$','','songxin_check')
    SetTriggerOption("xin_check1","group","xin_check")
    --SetTriggerOption("xin_check2","group","xin_check")
	mail_cnt=0
...
--[[	
    ...
    exe('i') 
    exe('alias action 检查信是否还在')	
	create_timer_s('songxin_check_timer',1.0,'songxin_check_test')
	]]--
...
function songxin_check()   
    --DeleteTimer('songxin_check_timer')
    print('songxin_check...........是否还有信呢？') 
    --if Bag["信件"] then
	print('mail_cnt'..mail_cnt)
	if mail_cnt==1 then
	   mail_cnt=0
	   EnableTriggerGroup("xin_check",false)
...
function songxin_act_set()
    ...
    Execute('halt;songxin '..sxjob.id..' '..sxjob.cnt)
end
...
function songxin2_consider()
    print('songxin2_consider.........................sxjob.sx2:'..sxjob.sx2)
    EnableTriggerGroup("songxin2_killer",true)
	if sxjob.sx2==0 then return sx2overok() end
	if not flag.sx2 and sxjob.sx2==1 then 
	    print('sx2 refuese.....')
...
function songxin2_refuse()
    ...
    exe('no')
    sxjob.sx2=0
	create_timer_s('walkWait4',1.0,'songxin2_refuse_timer')
end

function songxin2_refuse_timer()
   exe('no')
end 
